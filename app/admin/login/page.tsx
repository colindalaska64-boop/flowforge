...
export default function AdminLoginPage() {
  ...
  async function sendCode() {
    ...
    setLoading(false);
    if (res.ok) { setSent(true); setStep("verify"); }
          ...
          <div>
            <div style={{ background:"rgba(129,140,248,0.08)", border:"1px solid rgba(129,140,248,0.2)", borderRadius:10, padding:"1rem", marginBottom:"1.5rem" }}>
              <p style={{ fontSize:".82rem", color:"rgba(255,255,255,0.6)", lineHeight:1.6 }}>
                ...
                <strong style={{ color:"#818CF8" }}>{session?.user?.email}</strong>
              </p>
            </div>
          ...
          <form onSubmit={verifyCode}>
            <div style={{ background:"rgba(5,150,105,0.08)", border:"1px solid rgba(5,150,105,0.2)", borderRadius:10, padding:"1rem", marginBottom:"1.5rem" }}>
              <p style={{ fontSize:".82rem", color:"rgba(255,255,255,0.6)" }}>
                Code envoyé à <strong style={{ color:"#34D399" }}>{session?.user?.email}</strong> — valable <strong style={{ color:"#34D399" }}>10 minutes</strong>
              </p>
            </div>
            <input
              type="text"
            ...
            <button type="button" onClick={() => { setStep("send"); setCode(""); setError(""); setSent(false); }} style={{ width:"100%", background:"none", border:"none", color:"rgba(255,255,255,0.3)", fontSize:".8rem", cursor:"pointer", fontFamily:"inherit" }}>
